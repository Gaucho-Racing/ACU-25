...
/// @return 0 for success, 1 for FAILURE ðŸ¤¦â€â™‚ï¿???
int setup(){
  ...
  if (counter == 0){
    state = SHITDOWN;
    print_lpuart("â•­âˆ©ï¿????( â€¢ï¿½?_Â·Ì )â•­âˆ©ï¿???? [Failed BCC_Init...]\n");
    return -1;
  }
  print_lpuart("ðŸ’Ž [Successful BCC_Init...]\n");
...
int main(void)
{
  ...
  if(!state_system_check(true, true)){
    state = SHITDOWN;
    print_lpuart("(,,>ï¿???<,,) [SysCheck failed. Shutting down]\n");
  }
  else state = STANDBY;
  ...
  while (1)
  {
    ...
    if(acu.ts_active && state == STANDBY){
      ...
      print_lpuart("(,,>ï¿???<,,) [ts_active command, but state > STANDBY]\n");
      state = PRECHARGE;
    }
    else if(!acu.ts_active){ // not sure if this is the correct move
      print_lpuart("(,,>ï¿???<,,) [ts_active = 0; SHUTDOWN]\n");
      state = SHITDOWN;
    }
